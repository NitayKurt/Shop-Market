...
export default function ShopList() {
  ...
  const itemsRef = collection(database, "market-list").firestore;
  const [data, setData] = useState([]);
  const docReference = doc(database, "market-list", "products");

  const getData = async () => {
    // try {
    //   const querySnapshot = await getDocs(collection(database, "market-list"));
